...
#import "RSGoogleTranslate.h"

@interface RSGoogleTranslate()
...
@implementation RSGoogleTranslate
...
- (void)getTokenWithText:(NSString *)text completion:(void (^)(NSString * _Nullable tokenData, NSString * _Nullable error))completionHandler {
    ...
    NSString *url = [NSString stringWithFormat:@"https://api.yooul.net/api/google/token?text=%@&tkk=%@", [self urlEncode:text], _tkk];
...
//        }];
    ...
    dispatch_group_notify(_dispatchGroup, globalQueue, ^{
        self.data[@"q"] = [self urlEncode:text];
...
- (NSString *)urlEncode:(NSString *)unEncodedStr {
    ...
    encodedString = [unEncodedStr stringByAddingPercentEncodingWithAllowedCharacters:allowedCharacters];
